...
def pad_my(img, shp,stride, mode='constant', constant_values=0):
    if shp[2][0]==shp[2][1]==shp[3][0]==shp[3][1]==0: return img
    (n, c, h, w), (mn, mc, mh, mw) = img.shape, shp
    newh = h+mh[0]*2-h%stride[0]
    neww = w+mw[0]*2-w%stride[1]
    newimg = np.zeros((n, c, newh, neww), dtype=img.dtype)
    try:
        newimg[:, :, mh[0]:newh-mh[1] + h % stride[0], mw[0]:neww -mw[1] + w % stride[1]] = img
    except:
        debug = 1
    return newimg
...
def conv1(img, core, group=1, stride=(1, 1), dilation=(1, 1)):
    (strh, strw), (dh, dw) = stride, dilation
    (n, c, h, w), (ni, ci, hi, wi)  = core.shape, img.shape
    cimg_w = c * h * w * group
    cimg_h, i = ni*(hi//strh+hi%strh)*(wi//strw+wi%strw), 0
    shp = ((0, 0), (0, 0), (dh*(h//2),)*2, (dw*(w//2),)*2)
    img = pad(img, shp, 'constant', constant_values=0)
    # img = pad_my(img, shp,stride, 'constant', constant_values=0)
    img = img.transpose((2,3,0,1)).copy()
    col_img = np.zeros((w*h, hi//strh+hi%strh, wi//strw+wi%strw, ni, ci), img.dtype)
    for r in range(0, h * dh, dh):
        for c in range(0, w * dw, dw):
            try:
                col_img[i], i = img[0 + r:hi + r:strh+hi%strh, 0 + c:wi + c:strw+wi%strw], i + 1
            except:
                debug = 1
    col_img.shape = (w*h, cimg_h, -1, ni, group, ci)
    col_img = col_img.transpose((3,4,1,2,5,0))
    ...
    rst = [i.dot(j.T) for i, j in zip(col_core, col_img)]
    rst = rst[0] if group==1 else np.concatenate(rst)
    return rst.reshape((ni, n, hi//strh+hi%strh, wi//strw+wi%strw))
